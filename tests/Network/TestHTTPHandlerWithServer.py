...
class TestHTTPHandlerWithServer(unittest.TestCase):
    ...
    def test_start_new_game_invalid_params(self):
        with self.assertRaises(requests.exceptions.HTTPError):
            response = self.handler.start_new_game('player1', 10, 9)  # Invalid positions and colors
            self.print_response(response)
    ...
    def test_make_move_invalid_params(self):
        with self.assertRaises(requests.exceptions.HTTPError):
            response = self.handler.make_move(1, 'player1', 10, 9, '1234')  # Invalid positions and colors
            self.print_response(response)
